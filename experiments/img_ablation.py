#!/usr/bin/env python
"""
Image Classification Ablation Experiment
========================================
ViT-B/16에서 LAVA 하이퍼파라미터 민감도 분석
병렬 GPU 실행 지원
"""
...
class ImageAblationRunner(BaseExperimentRunner):
    """Image Classification 태스크에서 LAVA ablation 실험 (병렬 GPU 실행 지원)"""
    ...
    def __init__(self, seeds=None, gpus="0", per_gpu_tasks=1, test_mode=False,
        ...
        super().__init__(
            ...
            use_wandb=use_wandb,
            wandb_project=wandb_project or "IMG-Ablation",
        )
        self._tasks = tasks if tasks else IMG_TASKS

    @property
    def csv_columns(self):
        return IMG_ABLATION_CSV_COLUMNS
    ...
    def run_single_experiment(self, task: str, seed: int,
        ...
        result_file = self.result_dir / f"img_result_{task}_s{seed}_vib{vib}_stab{logit_stab}_lat{latent_stab}.json"
    ...
    def run_all_experiments(self, param_types=None):
        ...
        self.log(f"")
        self.log(f"{'='*60}")
        self.log(f" Image Ablation 완료!")
        self.log(f" 결과: {self.csv_path}")
        self.log(f"{'='*60}")


def main():
    parser = argparse.ArgumentParser(description="Image Ablation Experiment (병렬 GPU 지원)")
    ...
    parser.add_argument("--wandb_project", type=str, default="IMG-Ablation")
